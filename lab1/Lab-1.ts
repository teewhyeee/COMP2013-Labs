...
/**
 * Task-2:
 * Make a variable named listing0, of type Listing and assign the first listing object
 ...
 * using the spread operator to copy listing0.
 * Then add 2 new properties,
 * isSold which is a boolean set to false,
 * and currentOwner which is a string with the value of Jane Doe
 * NOTE: This will cause an error with the Listing interface as isSold and currentOwner
 * are not current present in the definition of the Listing interface.
 ...
 * Task-4:
 * Create a function to calculate the realtor fees.
 * The function's name is realtorFees, that takes a Listing object as it's input parameter,
 * and returns a number with the dollar amount as realtor fees using the following conditions,
 ...
 * for all houses that are only "Townhouse".
 * Then store it in a variable named townhouseListings
